    ...
    override func didMove(to view: SKView) {
        ...
        let reminderIfIdle = SKAction.run {
            if self.egg_correctTouches == 0 && self.egg_incorrectTouches == 0 {
                ...
                let egg_reminder = SKAction.playSoundFileNamed("instructions_hamburger", waitForCompletion: true)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if (instructionsComplete == true) && (reminderComplete == true) {
            ...
            if (egg?.contains(touch.location(in: self)))! {
                ...
                egg?.texture = SKTexture(imageNamed: "burgerColored")
                ...
                run(sequenceFade) {
                    let airplaneScene = SKScene(fileNamed: "DuckScene")
