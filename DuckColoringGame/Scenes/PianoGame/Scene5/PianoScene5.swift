...
class PianoScene5: SKScene {
    ...
    var piano5_incorrectTouches = 0
    var piano5_correctTouches = 0
    ...
    override func didMove(to view: SKView) {
        ...
        let instructions = SKAction.playSoundFileNamed("instructions_piano5", waitForCompletion: true)
        run(instructions, completion: { self.instructionsComplete = true })
        
        // If the scene has not been touched for 10 seconds, play the reminder instructions; repeat forever
        let timer = SKAction.wait(forDuration: 10.0)
        let reminderIfIdle = SKAction.run {
            if self.piano5_correctTouches == 0 && self.piano5_incorrectTouches == 0 {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if (instructionsComplete == true) && (reminderComplete == true) {
            ...
            if (targetItem?.contains(touch.location(in: self)))! {
                ...
                piano5_correctTouches += 1
                correctTouches += 1
                // Play audio feedback for the correct touch
                let correctTouch = SKAction.playSoundFileNamed("correctAudio5", waitForCompletion: true)
                ...
                run(sequenceFade) {
                    let piano6 = SKScene(fileNamed: "PianoScene6")
                    piano6?.scaleMode = SKSceneScaleMode.aspectFill
                    self.scene!.view?.presentScene(piano6!)
                }
                
            }
            else {
                // Else, award points for incorrect touch
                piano5_incorrectTouches += 1
                incorrectTouches += 1
            }
            
            // play reminder instructions if user has touched screen 3 times incorrectly
            if piano5_incorrectTouches == 3 && piano5_correctTouches < 1 {
                reminderComplete = false
                // ********THIS AUDIO JUST REPEATS ORIGINAL INSTRUCTIONS, INSTEAD OF REPHRASING. PLEASE REQUEST A REMINDER AUDIO FROM YAO
                let reminder_piano5 = SKAction.playSoundFileNamed("instructions_piano5", waitForCompletion: true)
                run(reminder_piano5, completion: { self.reminderComplete = true} )
            }
