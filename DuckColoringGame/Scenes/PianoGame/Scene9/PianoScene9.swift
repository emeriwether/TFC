...
class PianoScene9: SKScene {
    ...
    var piano9_incorrectTouches = 0
    var piano9_correctTouches = 0
    ...
    override func didMove(to view: SKView) {
        ...
        let reminderIfIdle = SKAction.run {
            if self.piano9_correctTouches == 0 && self.piano9_incorrectTouches == 0 {
                ...
                self.run(reminder_piano9, completion: { self.reminderComplete = true} )
            }
        }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if (instructionsComplete == true) && (reminderComplete == true) {
            ...
            if (targetItem?.contains(touch.location(in: self)))! {
                ...
                run(sequenceFade) {
                    let allDone = SKScene(fileNamed: "StartScene")
                ...
                piano9_incorrectTouches += 1
                //incorrectTouches += 1
            }
            
            // play reminder instructions if user has touched screen 3 times incorrectly
            if piano9_incorrectTouches == 3 && piano9_correctTouches < 1 {
