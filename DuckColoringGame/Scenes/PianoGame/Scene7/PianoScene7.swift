...
class PianoScene7: SKScene {
    ...
    var piano7_incorrectTouches = 0
    var piano7_correctTouches = 0
    ...
    override func didMove(to view: SKView) {
        ...
        let instructions = SKAction.playSoundFileNamed("instructions_piano7", waitForCompletion: true)
        run(instructions, completion: { self.instructionsComplete = true })
        
        // If the scene has not been touched for 10 seconds, play the reminder instructions; repeat forever
        let timer = SKAction.wait(forDuration: 10.0)
        let reminderIfIdle = SKAction.run {
            if self.piano7_correctTouches == 0 && self.piano7_incorrectTouches == 0 {
                ...
                let reminder_piano7 = SKAction.playSoundFileNamed("instructions_piano7", waitForCompletion: true)
                self.run(reminder_piano7, completion: { self.reminderComplete = true} )
            }
        }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if (instructionsComplete == true) && (reminderComplete == true) {
            ...
            if (targetItem?.contains(touch.location(in: self)))! {
                ...
                run(sequenceFade) {
                    let piano8 = SKScene(fileNamed: "PianoScene8")
                    piano8?.scaleMode = SKSceneScaleMode.aspectFill
                    self.scene!.view?.presentScene(piano8!)
                }
                
            }
            else {
                // Else, award points for incorrect touch
                piano7_incorrectTouches += 1
                incorrectTouches += 1
            }
            
            // play reminder instructions if user has touched screen 3 times incorrectly
            if piano7_incorrectTouches == 3 && piano7_correctTouches < 1 {
                reminderComplete = false
                // ********THIS AUDIO JUST REPEATS ORIGINAL INSTRUCTIONS, INSTEAD OF REPHRASING. PLEASE REQUEST A REMINDER AUDIO FROM YAO
                let reminder_piano7 = SKAction.playSoundFileNamed("instructions_piano7", waitForCompletion: true)
                run(reminder_piano7, completion: { self.reminderComplete = true} )
            }
